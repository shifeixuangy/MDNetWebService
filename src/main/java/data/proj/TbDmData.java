 ...
 */
@Entity
@Table(name = "tb_dm_data", schema = "")
@JsonNaming(PropertyNamingStrategy.PascalCaseStrategy.class)
public class TbDmData  implements SensorData{
    @JsonProperty("I")
    private int id;
    @JsonProperty("S")
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof TbDmData)) return false;

        TbDmData tbDmData = (TbDmData) o;

        if (Double.compare(tbDmData.dispA, dispA) != 0) return false;
        if (Double.compare(tbDmData.dispB, dispB) != 0) return false;
        if (id != tbDmData.id) return false;
        if (sensorId != tbDmData.sensorId) return false;
        if (Double.compare(tbDmData.velocityA, velocityA) != 0) return false;
        if (Double.compare(tbDmData.velocityB, velocityB) != 0) return false;
        if (!time.equals(tbDmData.time)) return false;
